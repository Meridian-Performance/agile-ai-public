...
from agile_ai.estimation.distance.beam_signed_distance import BeamSignedDistance
...
def beam_signed_distance_test():
    ...
    def _():
        @it("returns the signed distance starting from half width from the center beam")
        def _(tc: TestContext):
            vector = np.array([3.0, 5.0])
            normal = normed(vector)
            point = np.array([6.0, 10.0])
            lsd = BeamSignedDistance(point, vector, width=3.0)
            points = np.array([
                point + normal * -1.55,
                point + normal * -1.35,
                point,
            ...
            expect(distances).to_be(close_to_array(np.array([+0.05, -0.15, -1.5, -0.25, +2.05])))
